...
{
    public static class BitmapMaskRegion
    {
        //This method uses only safe code (no pointers) to scan an imagine
        //and create a region from it
        public static Region CreateSlow(Bitmap inputBmp, Color transparencyKey, int tolerance)
        {
            ...
            for (int x = 0; x < inputBmp.Width; x++)
                for (int y = 0; y < inputBmp.Height; y++)
                    if (!colorsMatch(inputBmp.GetPixel(x, y), transparencyKey, tolerance))
        ...
        public static  Region GetRegion(Bitmap bitmap, Color transparencyKey)
        {
            Color _matchColor = Color.FromArgb(transparencyKey.R, transparencyKey.G, transparencyKey.B);
            ...
            for (int y = 0; y < bitmap.Height; y++)
            {
                for (int x = 0; x < bitmap.Width; x++)
                {
                    if (!inimage)
                    {
                        if (bitmap.GetPixel(x, y) != _matchColor)
                        {
                            inimage = true;
                            rc.X = x;
                            rc.Y = y;
        ...
        public unsafe static Region CreateFast(Bitmap bitmap, Color transparencyKey, int tolerance)
        {
            //Bounds
